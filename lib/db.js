...
class Datastore {
   ...
   * @param {Boolean} getDocs Whether full doc should be fetched or only return keys
   * @returns {Promise}
   */
  filter = (namespace, query, getDocs=false) => {
    const self = this
  ...
  list = (namespace, max) => {
    return this._storage.listDocs(this._bucket, namespace, max).then(res => {
      let getKeys = res.results.map(key => this.get(namespace, key).catch(noop))
      
      return Promise.all(getKeys).then(data => {
        return {
          next: data.NextContinuationToken, 
          results: data.filter(d => d != null)
        }
      })
    })
  }
